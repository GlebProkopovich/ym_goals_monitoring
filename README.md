...
Если сумма срабатываний за 10 дней с вчера (включая вчера) **≤ 5** — цель не проверяется на поломку; в `ym_goals_statuses` ставится **`inactive`** без уведомлений. Когда срабатываний снова достаточно, применяется обычная логика `active` / `broken`.

### Категории (эталон без вчера)

| Категория | Эталон (при вчера = 20 мая) | Условие | Окно проверки поломки |
|-----------|----------------------------|---------|------------------------|
| high | 10–19 мая | ≥ 50 | только вчера |
| mid | 9–18 мая | ≥ 20 и < 50 | вчера и позавчера |
| low | 8–17 мая | < 20 (при screening > 5) | 3 дня до вчера включительно |
...
- `inactive` — мало трафика (screening ≤ 5), без уведомлений
...
Все HTTP-запросы идут через `utils/http_retry.py` (3 попытки, экспоненциальная пауза, повтор при 429/5xx).
...
- Факты: `stat/v1/data`, период 13 дней, `ym:s:sumGoalReachesAny`
